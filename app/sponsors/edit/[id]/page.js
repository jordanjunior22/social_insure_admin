...
export default function Page() {
    ...
    async function handleUpdate() {
        try {
            const promise = new Promise(async (resolve, reject) => {
                const CreateResponse = await fetch('/api/sponsors', {
                    method: 'PUT',
                    headers: {
                ...
                if (CreateResponse.ok) {
                    resolve();
                    router.push('/sponsors'); // Assuming 'router' is defined elsewhere
                } else {
                    reject();
                }
            });
  ...
  return (
    <AdminLayout title='Sponsor > Edit Sponsor'>
        ...
        <div className='flex border bg-white w-1/2 rounded-md hover:shadow-md hover:shadow-[#18B8A8] items-center'>
            ...
            <input type='text' className='flex-1 px-1 outline-none rounded-r-md' value={website} onChange={(ev)=>{setWebsite(ev.target.value)}}/>
        </div>
